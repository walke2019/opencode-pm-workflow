...
```mermaid
flowchart TD
  A["OpenCode 会话开始或用户提出任务"] --> B["加载 pm-workflow 插件"]
  B --> C["合并配置：包默认 -> 全局配置 -> 插件参数 -> 项目配置"]
  C --> D["通过 OpenCode config hook 注入 agents"]
  D --> E["曹操 primary agent 可用"]
  D --> F["隐藏 subagents 可用：QA、文档、前端"]
  D --> G["保留 OpenCode 内置 agents：plan、build"]

  A --> H{"触发类型"}
  H -->|"session.created / session.idle"| I["observe 模式：同步状态与门禁"]
  H -->|"assist / strict 提示词追加"| J["向提示词追加阶段摘要"]
  H -->|"edit/write/apply_patch 后"| K["标记待审查并同步状态"]
  H -->|"slash 命令或工具调用"| L{"调度命令"}

  L -->|"pm-run-dispatch"| M["只返回推荐命令，不执行"]
  L -->|"pm-dry-run-dispatch / pm-dry-run-loop"| N["预演执行计划、权限、门禁、重试、回退"]
  L -->|"pm-execute-dispatch / pm-run-loop"| O{"确认、权限、门禁都通过？"}

  O -->|"否"| P["停止并返回阻塞原因"]
  O -->|"是"| Q["执行 opencode run 并指定 agent"]

  I --> R["生成状态摘要：文档、阶段、任务、审查、发布"]
  J --> R
  K --> R
  M --> R
  N --> R
  ...
  S -->|"需要产品/需求/统筹"| T["pm_workflow_caocao：主协调"]
  S -->|"需要开发计划"| U["OpenCode 内置 plan"]
  S -->|"需要真正实现"| V["OpenCode 内置 build"]
  S -->|"需要前端/UI 建议"| W["pm_workflow_frontend：隐藏 subagent"]
  S -->|"需要文档/发布说明"| X["pm_workflow_writer：隐藏 subagent"]
  S -->|"需要审查/质量门禁"| Y["pm_workflow_qa：隐藏 subagent"]
  ...
  V --> Z["代码改动由 OpenCode build 完成"]
  W --> AA["前端建议、交互审查、视觉验收点"]
  X --> AB["文档、变更记录、发布摘要"]
  Y --> AC["审查发现、风险、缺失测试"]
  T --> AD["澄清目标、范围、验收标准、下一步"]
  U --> AE["开发计划与任务拆分"]
  ...
  AF --> AG{"完成？"}
  AG -->|"否"| R
  AG -->|"是"| AH["进入 release-ready / completed 状态"]
```
