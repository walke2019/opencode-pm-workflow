...
function executeDispatchCommand(
  projectPath: string,
...
function getProjectDir(ctx: PluginContext) {
...
function buildStageSummary(projectDir: string) {
  const status = buildStateSummary(projectDir);
  return {
...
function buildReviewGateSummary(projectDir: string) {
  const markerPath = join(projectDir, REVIEW_MARKER_FILENAME);
...
function buildFeedbackSignalSummary(message: string) {
  ...
  return {
    tool: {
      ...
      "pm-get-execution-receipt": tool({
        ...
        async execute(args, context) {
          ...
          const receipts = getExecutionReceipts(projectPath, {
            ...
            success:
              args.success === "true" || args.success === "false"
                ? args.success
                : undefined,
          });
          return [
      ...
      "pm-get-execution-by-id": tool({
        ...
        async execute(args, context) {
          const projectPath = context.worktree || context.directory;
          const receipt = getExecutionReceiptById(
            projectPath,
            args.execution_id,
          );
          if (!receipt) {
      ...
      "pm-get-execution-summary": tool({
        description:
          "汇总 execution receipts 的成功率、最近 action 和最近 agent。",
        args: {
          limit: tool.schema
            .string()
            .optional()
      ...
      "pm-set-permission": tool({
        ...
        args: {
          key: tool.schema
            .string()
            .describe(
              "权限键：allow_execute_tools / allow_repair_tools / allow_release_actions",
            ),
        ...
        async execute(args, context) {
          ...
          if (!allowedKeys.includes(args.key as (typeof allowedKeys)[number])) {
          ...
          const next = setPermission(
            projectPath,
            args.key as (typeof allowedKeys)[number],
            args.value === "true",
          );

          return [
            "pm-workflow permission 已更新",
            `- ${args.key}: ${next.permissions[args.key as (typeof allowedKeys)[number]]}`,
          ].join("\n");
        },
      }),
      "pm-set-mode": tool({
        ...
        async execute(args, context) {
          const projectPath = context.worktree || context.directory;
          const allowedModes = ["off", "observe", "assist", "strict"] as const;
