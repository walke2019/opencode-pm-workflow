...
export const DEFAULT_HEALTH_THRESHOLDS: PluginHealthThresholds = {
  minAgents: 1,
  minTools: 5,
  minMcps: 0,
};
...
export function evaluatePluginHealth(input: {
  ...
  const thresholds: PluginHealthThresholds = {
 ...
 */
export async function reportPluginHealth(
  ctx: PluginContext,
 ...
 */
const ACTIVATED_PLUGIN_IDS = new Set<string>();

export type PluginActivationGuardResult = "first" | "duplicate";

export function guardPluginActivation(
  pluginId: string,
...
export function releasePluginActivation(pluginId: string) {
