...
function parseAgentFrontmatter(raw: string): {
  ...
  const result: { display_name?: string; theme?: string } = {};
  for (const line of fmText.split("\n")) {
    const sep = line.indexOf(":");
    if (sep === -1) continue;
    const key = line.slice(0, sep).trim();
    if (key !== "display_name" && key !== "theme") continue;
    const val = line
      .slice(sep + 1)
      .trim()
      .replace(/^["']|["']$/g, "");
 ...
 * 缺失文件 / 缺字段时优雅降级（不抛错）。
 */
export function readGlobalAgentsDisplayInfo(): AgentDisplayInfo[] {
 ...
 */
export function buildAgentThemeBannerContent(
  infos: AgentDisplayInfo[],
  ...
  try {
    ...
    await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () => reject(new Error("showToast timeout after 3s")),
          3000,
        ),
      ),
    ]);
