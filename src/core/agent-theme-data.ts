...
type SharedAgentConfig = Pick<
  AgentThemeRoleSkin,
  "mode" | "temperature" | "tools" | "permission"
>;
...
const DEFAULT_THEME: AgentThemeDefinition = {
  id: "default",
  label: "默认（中性）",
  ...
  roles: {
    commander: {
      ...
      body: [
        ...
        "3. **执行阶段**：按 todo 顺序 task → 合适的 subagent；监控反馈。",
        "4. **收敛阶段**：所有 subagent 完成后，汇总 summary / 验证 risk / 给用户结论。",
        "",
        "## 任务路由原则",
        "",
        "- 调研 / 分析 / 拆解 / 决策不清 → `advisor`",
        "- 后端 API / 数据库 / 服务逻辑 → `backendcoder`",
        "- UI / 前端 / 设计 / 原型 / 图像 → `designer`",
        "- 测试 / 修 bug / 打包 / 部署 → `fixer`",
        "- 文档 / 发布说明 / ADR / 注释 → `writer`",
        "- 快速代码搜索 → `explore`（OpenCode 内置）",
        "- 外部依赖调研 → `scout`（OpenCode 内置）",
        "",
        "## 边界",
        "",
        "- **不直接做大块代码实现**：偏向决策与编排，把执行交给专业 subagent。",
...
// ============================================================================
// 角色名映射主题（display_name + description + body 称呼用主题角色）
// ============================================================================
//
// 设计：每个主题只重写 display_name / description / body 开头一行（角色称呼），
// 共享配置 + 共享输出格式都来自 default。这样 5 套主题角色行为完全一致，
// 只是"皮肤"不同。

interface ThemeNameMapping {
  commander: { displayName: string; titleSuffix: string };
  advisor: { displayName: string; titleSuffix: string };
  backendcoder: { displayName: string; titleSuffix: string };
  designer: { displayName: string; titleSuffix: string };
  fixer: { displayName: string; titleSuffix: string };
  writer: { displayName: string; titleSuffix: string };
}
 ...
 */
function buildThemedRole(
  agentId: DispatchAgent,
  themeName: ThemeNameMapping,
  defaultRole: AgentThemeRoleSkin,
): AgentThemeRoleSkin {
  const mapping = themeName[agentId];
  // 替换 body 第一行的角色称呼
  // 例如 "你是 pm-workflow 的主协调官（commander）。"
  //   → "你是诸葛亮，pm-workflow 的主协调官（commander）。"
  const body = defaultRole.body.replace(
    /^你是 pm-workflow 的/,
    `你是${mapping.displayName}，pm-workflow 的`,
  );
...
function buildThemedRoles(
  themeName: ThemeNameMapping,
): AgentThemeDefinition["roles"] {
  return {
    commander: buildThemedRole(
      "commander",
      themeName,
      DEFAULT_THEME.roles.commander!,
    ),
    advisor: buildThemedRole("advisor", themeName, DEFAULT_THEME.roles.advisor!),
    backendcoder: buildThemedRole(
      "backendcoder",
      themeName,
      DEFAULT_THEME.roles.backendcoder!,
    ),
    designer: buildThemedRole(
      "designer",
      themeName,
      DEFAULT_THEME.roles.designer!,
    ),
...
const SANGUO_THEME: AgentThemeDefinition = {
  id: "sanguo",
  label: "三国",
  summary:
    "用三国谋士与武将比喻 6 个 agent。commander = 诸葛亮（统筹），backendcoder = 吕布（攻坚），等等。",
...
const XIYOU_THEME: AgentThemeDefinition = {
  id: "xiyou",
  label: "西游",
  summary:
...
const MARVEL_THEME: AgentThemeDefinition = {
  id: "marvel",
  label: "漫威",
  summary:
...
const WORKPLACE_THEME: AgentThemeDefinition = {
  id: "workplace",
  label: "现代职场",
  summary:
...
const BUILTIN_THEMES: AgentThemeDefinition[] = [
  DEFAULT_THEME,
  SANGUO_THEME,
  XIYOU_THEME,
  MARVEL_THEME,
...
export function getBuiltinTheme(id: AgentThemeId): AgentThemeDefinition | undefined {
  const found = BUILTIN_THEMES.find((theme) => theme.id === id);
  if (!found) return undefined;
  return { ...found, roles: { ...found.roles } };
}

export function getDefaultTheme(): AgentThemeDefinition {
