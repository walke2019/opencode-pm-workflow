...
export function readWorkflowConfig(
  ...
  try {
    ...
    const hasLegacyDispatch = Object.entries(
      merged.agents.dispatch_map,
    ).some(
      ([key, value]) =>
        key in LEGACY_TO_CURRENT ||
        (typeof value === "string" && value in LEGACY_TO_CURRENT),
    );
    const hasLegacyFallback = Object.entries(
      merged.fallback.agent_map,
    ).some(
      ([key, value]) =>
        key in LEGACY_TO_CURRENT ||
        (typeof value === "string" && value in LEGACY_TO_CURRENT),
    ...
    if (hasLegacyDefs || hasLegacyDispatch || hasLegacyFallback) {
      ...
      for (const [agentId, executableId] of Object.entries(
        merged.agents.dispatch_map,
      )) {
        if (executableId && LEGACY_TO_CURRENT[executableId]) {
          merged.agents.dispatch_map[agentId as DispatchAgent] =
            LEGACY_TO_CURRENT[executableId];
        }
      }

      for (const [agentId, fallbackId] of Object.entries(
        merged.fallback.agent_map,
      )) {
