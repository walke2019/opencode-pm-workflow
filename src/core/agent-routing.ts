...
export type AgentTaskPermission = Partial<
  Record<string, TaskPermissionValue>
>;
...
function findAgentMarkdown(
  projectDir: string,
  agentId: string,
): { source: "project" | "global"; filePath: string; raw: string } | undefined {
  for (const search of getAgentSearchDirs(projectDir)) {
    if (!existsSync(search.dir)) continue;
 ...
 */
export function parseFrontmatterTaskPermission(raw: string): {
  taskPermission: AgentTaskPermission;
} {
  const block = extractFrontmatterBlock(raw);
  if (!block) return { taskPermission: {} };

  const lines = block.split(/\n/);
  const taskPermission: AgentTaskPermission = {};
  let inPermission = false;
  let inTask = false;
  let permissionIndent = -1;
  let taskIndent = -1;

  const lineIndent = (line: string) => line.match(/^(\s*)/)?.[1].length || 0;
    ...
    if (inPermission && indent > permissionIndent) {
      ...
      if (inTask) {
        if (indent <= taskIndent) {
          ...
          if (sep === -1) continue;
          const subagent = trimmed.slice(0, sep).trim();
          const value = trimmed
            .slice(sep + 1)
            .trim()
            .replace(/^["']|["']$/g, "");
...
export function isSubagentAllowedByDeclarativeRouting(input: {
  ...
  const value = input.routing.taskPermission[input.candidate];

  if (value === "deny") {
    return {
      allowed: false,
      reason: `permission.task[${input.candidate}]=deny in ${input.routing.filePath ?? "frontmatter"}`,
    };
  }
  if (value === "allow" || value === "ask") {
    return {
      allowed: true,
      reason: `permission.task[${input.candidate}]=${value}`,
    };
  }
