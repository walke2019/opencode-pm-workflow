...
> 日期：2026-04-30（初版）/ 2026-05-07（更新至 0.1.14）/ 2026-05-08（补充 0.1.15 文档一致性）  
...
本文档定义 `opencode-pm-workflow` 流程图的设计方案，确保图示准确反映当前版本（`0.1.15`）的真实实现。

> 说明：Command Lane、Primary/Subagent 分流等能力的**首次引入版本**仍是 `0.1.14`，因此下文相关历史描述继续保留 `0.1.14` 表述。
...
在 `0.1.14` 的流程与图示基础上，`0.1.15` 没有引入新的 lane 或新的调用语义，而是补充了以下与 handoff 质量相关的实现收口：

- **Compact Handoff Prompt**：handoff packet 改为 `mission / context / scope / acceptance / artifacts / responseFormat`
- **Structured Agent Response**：统一要求 specialist 输出 `summary / verification / risk`
- **Evaluator Tightening**：缺少结构化字段时，不再直接判定为完成
...
- 未反映 `evaluation -> auto-continue -> stop reason` 的 `0.1.15` 当前闭环
...
| 日期 | 版本 | 变更 |
|---|---|---|
| 2026-05-08 | 0.1.15 | 同步当前版本文档说明：补充 compact handoff、结构化回传与 evaluator 收紧说明，并保留 0.1.14 作为能力首次引入版本。 |
