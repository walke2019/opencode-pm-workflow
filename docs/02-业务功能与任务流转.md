...
> **OpenCode 内 / 外两种模式**：能力表中"诊断 / 审计 / 配置"类能力可在 OpenCode 外通过 `pmw` CLI 独立运行；"分派 / 编排 / Auto-continue"类能力**必须在 OpenCode 进程里**运行（依赖 OpenCode 的 LLM runtime + tool 协议 + 子进程编排）。详见 README "适用场景"段。
...
| 声明式路由 | 0.7.0 起，commander → specialist 路由可在 agent markdown frontmatter 的 permission.task 字段声明，三级优先级（deny > allow/ask > fallback） |
...
| 跨项目共享 agent 库 | 0.10.0 起，`pmw agents list / promote / doctor` 三件套；列出 / 复制项目 agent 到全局 / 检查 frontmatter 完整性 |
| 公开 API 锁定 | 0.12.0 起写入 tools/api-snapshot.json，1.0.3 为 135 个公开符号；prepare-publish 自动校验；删除/改名走 deprecation 周期（1.0.0 起 SemVer 强制） |
| 诊断工具 | 状态查询、健康检查、执行回执 |
...
| 日期 | 版本 | 变更 |
| --- | --- | --- |
| 2026-06-04 | 1.1.1 | doctor 不再因缺少可选 `preferred_session_id` 阻塞；dispatch 交接提示补齐 project-scoped 流程文档写入路径 |
| 2026-05-28 | 1.0.3 | 业务能力表更新公开 API 符号总量；补齐局部主题与官方模型写入的运维能力说明 |
| 2026-05-23 | 1.0.0-rc.1 | §2 核心业务能力开头增加"OpenCode 内 / 外两种模式"边界说明 |
| 2026-05-23 | 0.12.0 | 业务能力表新增"公开 API 锁定"项 |
| 2026-05-22 | 0.10.0 | 业务能力表新增"跨项目共享 agent 库"项 |
