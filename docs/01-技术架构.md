...
`0.7.0` 起新增 `src/core/agent-routing.ts` 模块，让 `pm_lead → specialist` 路由的真相从代码迁到 agent markdown frontmatter。
...
| 层 | 是否可改 | 说明 |
| --- | --- | --- |
| 语义 ID（pm_lead / pm_advisor / pm_backend / pm_frontend / pm_reviewer / pm_researcher） | **永不可改** | 是 dispatch_map / permission.task / registry 的锚点 |
