...
- **Subagent**（如 `designer` / `fixer` / `advisor`）：通过 `opencode task ...` 执行
...
| 层 | 是否可改 | 说明 |
| --- | --- | --- |
| 语义 ID（commander / advisor / backendcoder / designer / fixer / advisor） | **永不可改** | 是 dispatch_map / permission.task / registry 的锚点 |
