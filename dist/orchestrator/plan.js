...
function buildExecutionPlanSteps(dispatch) {
    ...
    if (dispatch.recommendedAction === "start-development" ||
        dispatch.recommendedAction === "continue-development") {
        return [
            {
                id: "build-primary",
                title: dispatch.recommendedAction === "start-development"
                    ? "开始实现当前计划"
                    : "继续当前开发任务",
...
export function buildDispatchCommand(projectDir, prompt, lane) {
    ...
    const invocationMode = targetAgent === "commander" ? "primary" : "subagent";
