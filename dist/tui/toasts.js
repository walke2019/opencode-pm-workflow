import { buildDispatchCommand, buildDispatchPlan, buildDoctorReport, buildExecutionPlan, buildExecutionSummary, buildExecutionGate, buildFallbackPlan, buildPermissionGate, buildRecoverySummary, buildRetryPlan, buildSafetyReport, buildStateSummary, getExecutionReceipts, getLastExecutionReceipt, getMigrationReport, queryHistory, readWorkflowConfig, setAutomationMode, setPermission, } from "../shared.js";
export function formatLaneToast(input) {
    ...
    return {
        ...
        message: `${lane} | risk=${risk} | automation=${automation} | review=${review} | ` +
            `${input.recommendedAgent}/${input.recommendedAction}`,
    };
}
function formatDispatchToast(dispatch) {
...
export function createToastHelpers(api, projectDir) {
    ...
    const showLaneToast = (lane, duration = 6500) => {
    ...
    const showDoctorToast = (duration = 7000) => {
        ...
        api.ui.toast({
            ...
            message: report.blockers[0] || report.warnings[0] || `stage=${report.stage}`,
            duration,
        });
    };
    const showHistoryToast = (duration = 7000) => {
        ...
        api.ui.toast({
            ...
            message: events.length
                ? events
                    .map((event) => `${event.type || "unknown"}${event.action ? `/${event.action}` : ""}`)
                    .join(" | ")
                : "当前没有历史事件。",
            duration,
        });
    ...
    const showConfigToast = (duration = 7000) => {
        ...
        api.ui.toast({
            ...
            message: `mode=${config.automation.mode} storage=${config.docs.storage_mode} ` +
    ...
    const switchModeToast = (nextMode, duration = 7000) => {
    ...
    const showMigrationReportToast = (duration = 8000) => {
        ...
        api.ui.toast({
            variant: report.docs.conflicts_count > 0 || report.feedback.conflicts_count > 0
                ? "warning"
                : "info",
            title: "pm-workflow migration",
            message: `docs copied=${report.docs.copied_count} conflicts=${report.docs.conflicts_count} | ` +
    ...
    const showSafetyReportToast = (duration = 8000) => {
        ...
        api.ui.toast({
            ...
            message: `execute=${report.permissions.allow_execute_tools} doctor=${report.doctor.ok} ` +
                `action=${report.dispatch.recommendedAction} safe=${report.safeToEnableExecute}`,
            duration,
        });
    };
    const showExecutePermissionToggleToast = (nextValue, duration = 8000) => {
        const report = buildSafetyReport(projectDir);
        const current = readWorkflowConfig(projectDir).permissions.allow_execute_tools;
    ...
    const showExecutionReceiptsToast = (duration = 8000) => {
        ...
        api.ui.toast({
            ...
            message: receipts.length
                ? receipts
                    .map((receipt) => `${receipt.action}/${receipt.executable_agent}/exit=${receipt.exitCode}`)
                    .join(" | ")
