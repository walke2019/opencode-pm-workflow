    ...
    return {
        ...
        "pm-set-permission": tool({
            ...
            args: {
                key: tool.schema
                    .string()
                    .describe("权限 key，仅支持 allow_execute_tools / allow_repair_tools / allow_release_actions"),
                value: tool.schema
                    .string()
                    .describe('布尔值字符串，仅支持 "true" 或 "false"'),
            },
            async execute(args, context) {
                if (!PERMISSION_KEYS.includes(args.key)) {
                    return [
                ...
                return [
                    ...
                    `- key: ${args.key}`,
                    `- value: ${next.permissions[args.key]}`,
                ].join("\n");
            },
        }),
        "pm-set-mode": tool({
            ...
            async execute(args, context) {
                if (!AUTOMATION_MODES.includes(args.mode)) {
                    return [
