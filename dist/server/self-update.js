...
function detectCacheTarget() {
    const home = homedir();
    if (!home || home === "/" || home === "\\")
        return null;
    return join(home, ".cache", "opencode", "packages", `@walke/opencode-pm-workflow@latest`, "node_modules", PKG_NAME);
}
function readPluginMeta() {
    try {
        const p = join(homedir(), "Library", "Application Support", "ai.opencode.desktop", "opencode", "plugin-meta.json");
...
export async function checkAndLogSelfUpdate(ctx) {
    const result = checkSelfUpdate();
    if (result.status === "up-to-date") {
        await log(ctx.client, "debug", "pm-workflow self-update", result);
        return;
    }
    await log(ctx.client, result.status === "updated" ? "info" : "warn", "pm-workflow self-update", result);
}
export function checkSelfUpdate() {
    ...
    try {
        ...
        if (targetMeta || key in meta) {
            ...
            entry.load_count = (entry.load_count ?? 0) + 1;
