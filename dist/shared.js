...
function detectDocs(projectDir) {
...
function detectHasCode(projectDir) {
...
export function appendHistory(projectDir, payload) {
    ensureStateDir(projectDir);
    const historyPath = getHistoryPath(projectDir);
    appendFileSync(historyPath, `${JSON.stringify({ at: nowIso(), ...payload })}\n`, "utf-8");
}
...
export function queryHistory(projectDir, options = {}) {
...
export function getLastFailure(projectDir) {
    const events = readHistory(projectDir).reverse();
    return (events.find((event) => typeof event.exitCode === "number" && event.exitCode !== 0) || null);
}
export function buildRecoverySummary(projectDir) {
    const events = readHistory(projectDir);
    const dispatchFailures = events.filter((event) => event.type === "dispatch.executed" && event.exitCode !== 0);
    const fallbackEvents = events.filter((event) => event.type === "fallback.executed");
    const stageTransitions = events.filter((event) => event.type === "stage.transition");
...
function ensureHistoryBootstrap(projectDir, state) {
    const historyPath = getHistoryPath(projectDir);
    if (existsSync(historyPath))
        return;
    appendHistory(projectDir, {
...
export function buildRetryPlan(projectDir, action) {
    const state = readState(projectDir);
    const config = readWorkflowConfig(projectDir);
...
function getFallbackAgent(action, executableAgent, config) {
    if (!config.fallback.enabled_actions.includes(action))
        return null;
    return config.fallback.agent_map[executableAgent] || null;
}
export function buildFallbackPlan(projectDir, action, executableAgent) {
    const state = readState(projectDir);
    const config = readWorkflowConfig(projectDir);
    ...
    const attempts = fallback.action === action && fallback.from_agent === executableAgent
        ? fallback.attempts
        : 0;
    const maxAttempts = config.fallback.max_attempts || fallback.max_attempts || 1;
...
export function recordFallbackExecution(projectDir, input) {
    const previous = readState(projectDir);
    const config = readWorkflowConfig(projectDir);
    const success = input.exitCode === 0;
    const attempts = previous.fallback.action === input.action &&
    ...
    const next = {
        ...
        fallback: success
            ...
            : {
                ...
                action: input.action,
                attempts,
                max_attempts: config.fallback.max_attempts || previous.fallback.max_attempts || 1,
                last_error: input.stderr?.slice(0, 1000) ||
...
export function recordDispatchExecution(projectDir, input) {
    const previous = readState(projectDir);
    const config = readWorkflowConfig(projectDir);
    const success = input.exitCode === 0;
    const docs = detectDocs(projectDir);
    let nextStage = previous.stage;
    let nextPhaseStatus = success
    ...
    if (success) {
        ...
        else if (input.action === "create-design-brief" &&
            docs.product_spec &&
            docs.design_brief) {
        ...
        else if ((input.action === "start-development" ||
    ...
    const next = {
        ...
        retry: success
            ? defaultRetryState()
            : {
                status: previous.retry.action === input.action &&
                    previous.retry.attempts + 1 >= config.retry.max_attempts
                    ? "exhausted"
                    : "pending",
                action: input.action,
                attempts: previous.retry.action === input.action
                    ? previous.retry.attempts + 1
                    : 1,
                max_attempts: config.retry.max_attempts || previous.retry.max_attempts || 1,
                last_error: input.stderr?.slice(0, 1000) ||
                    input.stdout?.slice(0, 1000) ||
                    null,
                last_exit_code: input.exitCode,
...
export function buildDispatchPlan(projectDir) {
    const state = buildStateSummary(projectDir);
    const gates = buildGateSummary(projectDir);
    let recommendedAgent = "pm";
...
function getExecutableAgent(agent) {
    if (agent === "build")
        return "build";
    return "plan";
}
function buildExecutablePrompt(agent, prompt) {
    if (agent === "pm") {
...
export function buildFallbackCommand(projectDir, dispatch, fallbackAgent, prompt) {
