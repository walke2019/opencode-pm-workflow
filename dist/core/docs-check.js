...
function findCurrentReadmeVersion(readme) {
    const match = readme.match(/当前发布版本：`([^`]+)`/);
    return match?.[1];
}
function buildFinding(severity, name, detail) {
    return { severity, name, detail };
}
function listMarkdownDocs(projectDir) {
 ...
 */
export function buildDocsCheckReport(projectDir) {
    ...
    for (const relativePath of MAIN_DOCS) {
        ...
        checks.push(buildFinding(exists ? "ok" : "blocker", `main-doc:${relativePath}`, exists ? "主文档存在" : "主文档缺失"));
    }
    ...
    if (existsSync(readmePath)) {
        ...
        checks.push(buildFinding(readmeVersion === packageVersion ? "ok" : "blocker", "readme-version", readmeVersion === packageVersion
    ...
    if (existsSync(changelogPath)) {
        ...
        checks.push(buildFinding(changelog.includes(`## ${packageVersion}`) ? "ok" : "blocker", "changelog-version", changelog.includes(`## ${packageVersion}`)
            ? `CHANGELOG 包含 ${packageVersion} 版本节`
            : `CHANGELOG 缺少 ${packageVersion} 版本节`));
    }
    else {
        checks.push(buildFinding("blocker", "changelog-version", "CHANGELOG.md 缺失"));
    }
    for (const relativePath of MAIN_DOCS) {
        ...
        checks.push(buildFinding(text.includes("## Change Log") ? "ok" : "blocker", `change-log:${relativePath}`, text.includes("## Change Log")
            ? "文档包含 Change Log"
            : "文档缺少 Change Log"));
    }
    for (const relativeDir of FORBIDDEN_DOC_DIRS) {
        const dirPath = join(projectDir, relativeDir);
        checks.push(buildFinding(existsSync(dirPath) ? "blocker" : "ok", `forbidden-dir:${relativeDir}`, existsSync(dirPath)
    ...
    for (const relativePath of MAIN_DOCS) {
        ...
        if (!existsSync(filePath))
            continue;
        const text = readText(filePath);
        const hits = FORBIDDEN_REFERENCE_PATTERNS.filter((pattern) => text.includes(pattern));
        checks.push(buildFinding(hits.length === 0 ? "ok" : "blocker", `legacy-reference:${basename(relativePath)}`, hits.length === 0
            ? "未发现旧文档路径引用"
            : `发现旧文档路径引用: ${hits.join(", ")}`));
    }
